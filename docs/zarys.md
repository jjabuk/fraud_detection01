...
## 5. Jak to rozłożyć w czasie (Timeline)?

*   **Tydzień 1:** Kodowanie infrastruktury (OpenTofu, BQ), Data Ingestion w Dagsterze i EDA (Exploratory Data Analysis).
*   **Tydzień 2:** Logika wyciągania cech (Feature Engineering - najtrudniejsza intelektualnie część, zapobiegaj wyciekom).
*   **Tydzień 3:** Skrypt trenujący, kalibracja, spięcie tego z MLflow (którego możesz postawić testowo lokalnie i docelowo przenieść na chmurę).
*   **Tydzień 4:** Pisanie API w FastAPI, Dockeryzacja, Deployment na Cloud Run. Połączenie wszystkich kropek.
*   **Tydzień 5:** Pucowanie GitHub Readme, dodanie testów, generowanie wykresów wyjaśnialności modelu (**SHAP**), podsumowanie wyników biznesowych.
